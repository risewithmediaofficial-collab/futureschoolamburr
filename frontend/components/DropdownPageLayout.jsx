                ...
                {highlights.length > 0 && (
                  <div className="absolute bottom-6 left-6 right-6 z-20 hidden md:flex gap-4 justify-center">
                     {highlights.map((item, idx) => (
                       <div key={idx} className="bg-white/95 backdrop-blur-sm rounded-2xl px-6 py-4 shadow-lg border border-white/20 min-w-[200px] text-left">
  ...
  return (
    <div className={`grid items-start gap-6 ${columns}`}>
      {items.map((item, index) => (
        <DropdownCard key={`${item.title}-${index}`} title={item.title} eyebrow={item.eyebrow || `Section ${String(index + 1).padStart(2, '0')}`} tone={item.tone || 'white'}>
